...
def create_order(symbol: str, usd: float) -> Dict[str, Any]:
    session = get_session()
    try:
        # Get or create a default portfolio for loose orders
        default_portfolio = session.query(Portfolio).filter_by(name='Default Paper Portfolio').first()
        if not default_portfolio:
            default_portfolio = Portfolio(name='Default Paper Portfolio', initial_capital=100000.0, current_value=100000.0)
        ...
        o = Order(portfolio_id=default_portfolio.id, symbol=symbol.upper(), usd=usd, quantity=quantity, price=price, status='filled', executed_at=datetime.utcnow(), meta={'simulated': True})
        session.add(o)
        session.commit()
        return o.to_dict()
    except Exception:
