...
def create_order(symbol: str, usd: float) -> Dict[str, Any]:
    ...
    try:
        ...
        # Ensure a default portfolio exists for paper trading orders
        portfolio = session.query(Portfolio).filter_by(name='Default Paper Portfolio').first()
        if not portfolio:
            portfolio = Portfolio(name='Default Paper Portfolio', initial_capital=100000.0, risk_profile='moderate')
