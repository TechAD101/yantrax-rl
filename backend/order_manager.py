...
def create_order(symbol: str, usd: float, portfolio_id: int = None) -> Dict[str, Any]:
    ...
    try:
        if portfolio_id is None:
            ...
            default_port = session.query(Portfolio).filter_by(name="Default Paper Portfolio").first()
            if not default_port:
                default_port = Portfolio(name="Default Paper Portfolio", initial_capital=100000.0, current_value=100000.0)
                session.add(default_port)
                session.flush()
            portfolio_id = default_port.id
