...
class WaterfallMarketDataService:
    ...
    def _init_providers(self):
        # 1. YFinance (Price Primary)
        self.providers['yfinance'] = {
            'enabled': True,
            'limiter': DailyRateLimiter('yfinance', 2000) # Soft limit to avoid ip ban
        }
        
        # 2. FMP (Fundamentals Primary)
        ...
        # 4. Alpha Vantage (Deep Backup - Strict Limit)
        av_key = os.getenv('ALPHA_VANTAGE_API_KEY')
        self.providers['alpha_vantage'] = {
            'enabled': bool(av_key),
            'key': av_key,
            'limiter': DailyRateLimiter('alpha_vantage', 25) # Standard free tier
        }
    ...
    def get_price(self, symbol: str) -> Dict[str, Any]:
        ...
        # Strategy 2: Alpaca (Reliable, Live)
