...
def test_backtest_strategy(client):
    ...
    strat_resp = client.post(
        ...
        data=json.dumps(
            {"name": "Test Strategy", "description": "Test", "archetype": "quant"}
        ),
...
def test_user_registration_weak_password(client):
    ...
    resp = client.post(
        "/api/auth/register",
        data=json.dumps(
            {"username": "testuser2", "email": "test2@example.com", "password": "short"}
        ),
        content_type="application/json",
    )
    assert resp.status_code == 400
    assert "least 8 characters long" in resp.get_json()["error"]
